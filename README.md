...
- ✅ **特征分析** - 标题特征（含高播放标题的 n-gram lift）、时长区间、发布时段、成功评分
...
- **互动率**：`(点赞 + 评论) / 播放量`
- **分位数 / 时长分桶**：深度分析面板中的 p25 / p50 / p75 与 0-15s / 15-30s / 30-45s / 45-60s / 60-180s 分布
- **标题特征**：除平均标题长度外，还会找出在“播放速度最快的那批视频”中过度出现的词、emoji、数字、疑问句（lift）。基于前 25% vs 其余的描述性对比，样本小时只当提示。
